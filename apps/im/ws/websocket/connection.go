...
func (c *Conn) keepalive() {
	...
	for {
		...
		case <-idleTimer.C:
			c.idleMu.Lock()
			idle := c.idle
			if idle.IsZero() { // the connection is no-idle  读消息
				c.idleMu.Unlock()
			...
			c.idleMu.Unlock()
			if val <= 0 {
				// the connection has already idle for a duration of keepalive.MaxIdle or more
				// graceful close the connection
				c.s.Close(c)
				return
			}
			idleTimer.Reset(val)
