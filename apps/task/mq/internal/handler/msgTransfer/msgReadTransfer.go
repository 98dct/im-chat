...
func NewMsgReadTransfer(svc *svc.ServiceContext) kq.ConsumeHandler {
	...
	if svc.Config.MsgReadHandler.GroupMsgReadHandler != GroupMsgReadHandlerAtTransfer {
		...
		if svc.Config.MsgReadHandler.GroupMsgReadRecordDelayTime > 0 {
			GroupMsgReadRecordDelayTime = time.Duration(svc.MsgReadHandler.GroupMsgReadRecordDelayTime)
		}
...
func (m *MsgReadTransfer) UpdateChatLogRead(ctx context.Context, data *mq.MsgMarkRead) (
	...
	for _, chatLog := range chatLogs {
		...
		//res[chatLog.ID.Hex()] = *(*string)(unsafe.Pointer(&chatLog.ReadRecords))
		err := m.svc.UpdateMarkRead(ctx, chatLog.ID, chatLog.ReadRecords)
		if err != nil {
			return nil, err
		}
	}
